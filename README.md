...
anota traços fenotípicos (metaTraits / EMBL), constrói uma base de dados piloto
(252 amostras, cohort `ERR4588xxx`) e permite comparar uma amostra nova contra essa BD,
inferindo um **perfil funcional** ("potencial microbiano inferido", não função
medida).
...
├─ data/
│  ├─ raw/
│  │  ├─ samples/                  252 relatórios Bracken (*.report_bracken.txt)
│  │  │   ├─ alentejo_samples/     cohort antigo de 47 (ERR156639xx) — arquivado, fora do glob
│  │  │   └─ excluded_degenerate/  23 amostras degeneradas excluídas — fora do glob
...
├─ docs/                 Especificação e tabelas de classificação de traços
...
5. `build_beta_diversity.py` — distâncias Bray-Curtis + PCoA + kNN (com projeção
   de amostra nova).
...
- BD atual = cohort `ERR4588xxx`, **252 amostras**. 23 amostras degeneradas
  (≤2 géneros, ≤279 reads) foram **excluídas** para `samples/excluded_degenerate/`;
  o cohort antigo de 47 (`ERR156639xx`) está arquivado em `samples/alentejo_samples/`.
```
